...
public class DAO_BookList {
	...
	public List<VO_BookList> booklist_search_list(String search, String key, int pagestart, int endpage) {
		String sql = "SELECT * FROM(SELECT ROWNUM RN,BOOKNO,BOOKNAME,BOOKTYPE,WRITER,REGDATE,LEND,LENDCNT,PUBLISHER, BOOKIMG FROM BOOKLIST WHERE "
				+ search + " LIKE '%" + key + "%' AND ROWNUM <= ? ORDER BY BOOKNO DESC) WHERE " + search + " like '%"
				+ key + "%' AND RN > ?";
		List<VO_BookList> list = new ArrayList<VO_BookList>();
		VO_BookList vo = null;
		try {
	...
	// bestsellers 출력 메소드 (5~8)
	public List<VO_BookList> booklistBestsellers_2page() {
		List<VO_BookList> list = new ArrayList<VO_BookList>();
		String sql = "select * from ( select a.*, rownum rnum from \r\n" + "   (\r\n"
				+ "     select * from booklist order by lendcnt desc   \r\n" + "   ) \r\n"
				+ "   a where rownum < 9)\r\n" + "where rnum > 4";
		VO_BookList vo = null;
		try {
			conn = getConnection();
	...
	// bestsellers 출력 메소드 (9~12)
	public List<VO_BookList> booklistBestsellers_3page() {
		List<VO_BookList> list = new ArrayList<VO_BookList>();
		String sql = "select * from ( select a.*, rownum rnum from \r\n" + "   (\r\n"
				+ "     select * from booklist order by lendcnt desc   \r\n" + "   ) \r\n"
				+ "   a where rownum <=12)\r\n" + "where rnum >= 9";
	...
	// 메인 bestsellers 출력 메소드 (1~6)
		public List<VO_BookList> mein_booklistBestsellers() {
			List<VO_BookList> list = new ArrayList<VO_BookList>();
			String sql = "SELECT * FROM(SELECT * FROM BOOKLIST ORDER BY LENDCNT DESC) WHERE ROWNUM <=6";
			VO_BookList vo = null;
			try {
				conn = getConnection();
				pstmt = conn.prepareStatement(sql);
	...
	public String booklist_reserveid(String bookno) {
		String sql = "select reserveid\r\n" + "from bookreserve\r\n" + "where reservestaus= '1' and bookno = ?";
		String reserve_memberid = "";
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);
			...
			rs = pstmt.executeQuery();
			if (rs.next()) {
				reserve_memberid = rs.getString("reserveid");
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	// 반납시 대출상태 변경 예약중
	public void booklist_return_reserve_lend(String bookno) {
		String sql = "update booklist SET lend=? where bookno = '" + bookno + "'";
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);
	...
	// 책 리스트 다운
	public List<VO_BookList> boolist_down() {
		List<VO_BookList> list = new ArrayList<VO_BookList>();
		String sql = "SELECT * FROM BOOKLIST ORDER BY bookno";
	...
	//
	public String booklist_modify_type(String bookno) {
		String sql = "select t.bookgenre\r\n" + 
				"from booklist l, booktype t\r\n" + 
				"WHERE l.booktype = t.booktype and bookno = ?";
		String bookgenre = "";
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);
