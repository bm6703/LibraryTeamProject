...
public class SqlGenerator_booklend {
	...
	static int iter = 30;				// 반복 횟수(생성 할 sql문 수)
	
	static String seq_name = "seq_booklend";
	
	static String lendid = "shss2216"; 	// 빌린사람 아이디
	static int maxBookNum = 10; 		// 책 갯수(10개)
	static int startYear = 2015;		// 시작년도
	...
	private static String makeSQL() {
		...
		for(int i = 0; i < iter ; i++) {
			...
			// sql문 한줄 생성
			String insertSql = "insert into booklend values("+seq_name+".nextval,'" + bookno + "','" +
					lendid + "','" +lenddate+"','"+lenddue+"','"+returndate+"', '0');\n";
