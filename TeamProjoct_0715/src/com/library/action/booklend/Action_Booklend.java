...
public class Action_Booklend implements Action {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (book_lend_max_num <= bookcnt) {
			...
			if (row == 1) {
				booklist_dao.booklist_lendCnt_lendtypesw(bookno); // 책 대여 성공시 lend상태 변경, 대여 수 증가
				// 예약자 존재여부
				String reserve = request.getParameter("reserve");
				if (reserve.equals("1")) {
					// 예약자 예약번호 가져오기
					String reservenum = dao.booklend_reservenum(memberid, bookno);
					// 예약자 책을 반납시 예약리스트에서 삭제
					dao.booklend_reserveid_delete(reservenum);
				}
				// sms
