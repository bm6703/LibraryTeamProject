...
<%!public String getRandom() {
		...
		for (int i = 0; i < 15; i++) {
			if (rnd.nextBoolean()) {
				if (rnd2.nextBoolean()) {
					random.append((char) ((int) (rnd.nextInt(26)) + 97));
				} else {
					random.append((char) ((int) (rnd.nextInt(26)) + 65));
				}
			} else {
					...
					<TR BGCOLOR="#FFFFFF">
						<TD ALIGN="center"><br>
							<form name="member" method="post"
								action="member?cmd=member_pwfound2"
								onsubmit="return formcheck();">

								<!-- 아이디 -->

								<label><b>아이디</b></label> <input type="text"
									placeholder="Enter ID" name="memberid" required class="id"
									id="memberid">
								...
								<div class="clearfix">
									...
									<td><input type="hidden" readonly="readonly"
										name="code_check" id="code_check" value="<%=getRandom()%>" /></td>
										...
										.blur(
												function() {
													...
													if (memberid == "") {
														...
														$
																.ajax({
																	url : 'member?cmd=member_idcheck2&memberid='
																			+ memberid,
																	type : 'get',
																	success : function(
																			data) {
