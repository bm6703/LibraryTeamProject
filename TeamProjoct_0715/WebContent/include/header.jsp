...
function calanderInsert(){
	   var url="admin?cmd=admin_calander_insert";
...
function member_login(){
	location.href="member?cmd=member_login";
}
function admin_login(){
	location.href="admin?cmd=admin_login";
}
function member_insert(){
	location.href="member?cmd=member_insert";
}
...
<div class="topnav">
				<ul>
					...
					<li><button type="button" class="btn btn-outline-link" onclick="admin_login();">관리자로그인</button></li>
					<li><button type="button" class="btn btn-outline-link" onclick="member_login();">로그인</button></li>
					</c:if>
					<!-- 세션이 관리자일 때(admin1) -->
					<c:if test="${not empty session && session=='admin1'}">
					<li><button type="button" class="btn btn-outline-link" onclick="admin_member_list();">권한설정</button></li>
					<li><button type="button" class="btn btn-outline-link" onclick="calanderInsert();">일정 입력</button></li>
					<li><button type="button" class="btn btn-outline-link" onclick="adminlogout();">로그아웃</button></li>
					</c:if>
					<!-- 세션이 이용자일 때 -->
					<c:if test="${not empty session && session!='admin1'}">
					<li><button type="button" class="btn btn-outline-link" onclick="memberlogout();">로그아웃</button></li>
					</c:if>
					<!-- 세션이 없을 때(로그인이 안되어있을 때) 회원가입메뉴 접근 -->
					<c:if test="${empty session}">		
					<li><button type="button" class="btn btn-outline-link" onclick="member_insert();">회원가입</button></li>
					</c:if>					
			...
			<div class="nav">
				<nav>
					<ul class="navi">
						...
						<li><a href="#">내서재</a>
							<ul class="navi2">
								<li><a href="booklend?cmd=booklend_list">대출/연장/예약</a></li>
								<c:if test="${not empty session && session!='admin1'}">
